...
class TumorClassificationHead(nn.Module):
    ...
    def _masked_pool(embedding: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        """Mean of the embedding over the mask voxels.

        embedding: (B, C, d, h, w)
        mask:      (B, 1, D, H, W) or (B, 1, d, h, w) -- resampled to the embedding grid.
        return:    (B, C)
        """
        ...
        # fallback: if the mask is empty, use global average pooling
    ...
    def forward(
        ...
        if self.pool == "masked":
            if mask is None:
                raise ValueError("pool='masked' requires a mask (B,1,D,H,W).")
