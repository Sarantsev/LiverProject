...
CLASSES = ["HCC", "ICC"]
SPATIAL = (8, 32, 32)   # small size for a fast test
...
def _make_fake_dataset(root):
    for cls in CLASSES:
        for pid in [f"{cls}_p001", f"{cls}_p002"]:   # unique patient ids
...
def test_missing_phase_padded():
    with tempfile.TemporaryDirectory() as tmp:
        root = os.path.join(tmp, "MCT")
        _make_fake_dataset(root)
        # remove one phase from one patient
        ...
        # find the patient with the missing phase
